...
<template>
  <nav class="fixed z-20 w-full border-b border-gray-100 bg-white">
    <div class="mx-auto flex max-w-screen-xl flex-wrap items-center justify-between gap-2 px-3 py-2">
      <form class="flex-1" @submit.prevent="onSubmit">
        <label for="top-navbar-search-input"
          class="sr-only mb-2 text-sm font-medium text-gray-900 dark:text-white">Buscar</label>
        <div class="relative">
          ...
          <input :value="search.search" @change.prevent="onInputChange" type="text" id="top-navbar-search-input"
            class="input block h-10 w-full p-2 pl-8 text-sm" placeholder="Buscar..." />
        </div>
      </form>
      ...
      <RouterLink :to="{ name: ROUTES.ORDERS }" class="flex cursor-pointer items-center space-x-3 ">
        <div class="rounded-xl p-2 transition-colors hover:bg-gray-100">
          <ShoppingBagOutline class="z-0 h-6 w-6 text-gray-600 transition-colors hover:text-gray-800" />
          <span v-if="showPulse" class="absolute right-2 top-4 inline-flex h-5 w-5 items-center justify-center">
