...
function onClickShare() {
	if (offer.value && isSupported.value) {
		...
		const price = offer.value.discount_price
...
<template>
	<main class="w-full container relative">
		<!-- Share button -->
		<button type="button" @click="onClickShare"
		...
		<template v-if="offer">
			<div class="fixed h-96 w-full bg-slate-500">
				<img :src="offer.image" :alt="offer.name" :title="offer.name" class="w-full h-full" @error="setDefaultImage" />
			</div>
			...
			<div class="min-h-[15rem] translate-y-96 space-y-4 rounded-t-3xl bg-white px-4 pb-36 pt-8">
				...
				<ul class="space-y-2.5 list-none p-2" v-if="offer.attributes?.length">
					...
					<li v-for="(attribute, index) in offer.attributes" :key="`single-offer-attribute-${index}`" class="flex gap-2">
